  ...
  describe.only('$refs', () => {
    ...
    it('string $refs (literal)', async () => {
      const src = {
        a: {
          ...
          $type: { $ref: '#/b/$type' },
          $value: { colorSpace: 'srgb', channels: [0.4, 0.2, 0.6] },
        },
        b: {
    ...
    it('local $refs + $defs + $type inheritance', async () => {
      const src = {
        ...
        $defs: {
          'red-1': { $value: { colorSpace: 'srgb', channels: [0.4, 0.1, 0] } }, // this is invalid as-written, but $defs shouldn’t be validated (only the final, resolved value)
        },
      };
      const config = defineConfig({}, { cwd });
      const { tokens } = await parse([{ filename: DEFAULT_FILENAME, src }], { config });
      expect(tokens['color.red.1']?.$value).toEqual({ colorSpace: 'srgb', channels: [0.4, 0.1, 0], alpha: 1 });
    });
